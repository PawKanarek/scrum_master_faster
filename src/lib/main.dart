...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void resetTimer() {
    setState((){
      ...
      pageModel.timer = new Timer.periodic(new Duration(seconds: 1), timerCallback);
    });
  }
  ...
  @override
  Widget build(BuildContext context) {
    TextStyle titleTextStyle = const TextStyle(fontSize: 20, color: Colors.black87);
    return Scaffold(
      appBar: AppBar(
      ...
      body:Container(
        ...
        child: Column(
          children: <Widget>[
            ...
            //   child: Text((pageModel.started) 
            //     ? "Standuper ${pageModel.currentPersonTalking + 1} przegadał ${getFormattedTime(pageModel.currentPersonElapsedMiliseconds)}"
            //     : 'Poranny stand-up czas rozpocząć',
            ...
            Expanded(child: new ListView.builder(
              itemCount: pageModel.peopleStanDupTimes.length,
              itemBuilder: (BuildContext context, int index){
                int value = pageModel.peopleStanDupTimes[index]; 
                String mood = (value < 61)?":)":(value < 121)?":|":":(";
                return ListTile(
                  dense: true,
                  title: Text("${index + 1}. ${getFormattedTime(value)}", style: new TextStyle(fontSize:13 + value.toDouble()/10)),
                  subtitle:  Text(mood, style: new TextStyle(fontSize:13))
                );
              })
            )
          ]),
...
class TimerTextState extends State<TimerText>{
  ...
  @override
  Widget build(BuildContext context){
    TextStyle timerTextStyle = const TextStyle(fontSize: 20, color: Colors.black87);
    return new Text("Czas standupu: ${getFormattedTime(pageModel.totalElapsedMiliseconds)}", style: timerTextStyle, textAlign: TextAlign.center);
  }
}

String getFormattedTime(elapsedMilisceconds){
    String minutes = (elapsedMilisceconds/ 60).floor().toString().padLeft(2, "0");
